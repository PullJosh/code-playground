      ...
      Program(path, state) {
        path.unshiftContainer(
          ...
          t.importDeclaration(
            ...
            t.stringLiteral(state.opts.diagnosticsURL || "https://example.com/")
          )
        );
      },
      "ExpressionStatement|VariableDeclaration|ReturnStatement": {
        /*
          For some reason the statements found inside of inline functions
          (functions defined within some other ExpressionStatement) don't
          work unless this runs on exit(). I don't understand Babel well
          enough to know why.
        */
        exit(path) {
